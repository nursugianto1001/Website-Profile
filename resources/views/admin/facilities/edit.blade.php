        ...
        <div class="p-6 bg-white border-b border-gray-200">
            ...
            <form action="{{ route('admin.facilities.update', $facility) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" required>{{ old('description', $facility->description) }}</textarea>
                    @error('description')
                        <span class="text-red-600 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    @if($facility->image_path)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $facility->image_path) }}" alt="{{ $facility->name }}" class="h-32 w-32 object-cover rounded">
                        </div>
                    @endif
                ...
                <div class="flex items-center justify-end mt-6">
                    ...
                    <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Update Facility</button>
                </div>
            </form>
        </div>
    </div>
