    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('admin.outlets.create') }}" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Add New Outlet</a>
            </div>
                ...
                <table class="min-w-full bg-white">
                    <thead>
                        <tr>
                            ...
                            <th class="py-3 px-4 border-b border-gray-200 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Address</th>
                    ...
                    <tbody>
                        @forelse($outlets as $outlet)
                            <tr>
                                <td class="py-4 px-4 border-b border-gray-200">
                                    <img src="{{ asset('storage/' . $outlet->image_path) }}" alt="{{ $outlet->name }}" class="h-16 w-16 object-cover rounded">
                                </td>
                                <td class="py-4 px-4 border-b border-gray-200">{{ $outlet->name }}</td>
                                <td class="py-4 px-4 border-b border-gray-200">{{ $outlet->address }}</td>
                                <td class="py-4 px-4 border-b border-gray-200">{{ $outlet->contact }}</td>
                                <td class="py-4 px-4 border-b border-gray-200">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('admin.outlets.edit', $outlet) }}" class="text-blue-600 hover:text-blue-900">Edit</a>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-4 px-4 border-b border-gray-200 text-center">No outlets found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
