...
<div class="relative z-30 bg-white" id="about">
    ...
    <div class="py-16 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12" data-aos="fade-up" data-aos-duration="800">
                <h2 class="text-3xl font-bold text-black">Poster Terbaru</h2>
                <p class="text-gray-500 mt-4">
                    Informasi terkini tentang event, promosi, dan kegiatan di Karvin Badminton
                </p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-8" data-aos="fade-up" data-aos-duration="800">
                @foreach($featuredPosters as $poster)
                <div class="bg-white rounded-lg overflow-hidden shadow-lg transition transform hover:scale-[1.02]">
                    <div class="h-60 overflow-hidden">
                        <img src="{{ asset('storage/' . $poster->image_path) }}" alt="{{ $poster->title }}"
                            class="w-full h-full object-cover">
                    </div>
                    <div class="p-6">
                        <h3 class="text-xl font-semibold mb-2">{{ $poster->title }}</h3>
                        @if($poster->description)
                        <p class="text-gray-600 text-sm">{{ Str::limit($poster->description, 100) }}</p>
                        @endif
                    </div>
                </div>
                @endforeach
    ...
    <div class="py-16 bg-gray-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12" data-aos="fade-up" data-aos-duration="800">
                <h2 class="text-3xl font-bold text-black">Galeri Dokumentasi</h2>
                <p class="text-gray-500 mt-4">
                    Potret keseruan dan momen berkesan di Karvin Badminton
                </p>
            </div>

            <div class="grid grid-cols-2 md:grid-cols-3 gap-4" data-aos="fade-up" data-aos-duration="800">
                @foreach($featuredDocumentations as $doc)
                <div class="overflow-hidden rounded-lg shadow-md h-60 transition transform hover:scale-[1.02]">
